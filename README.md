...
- [x] **F-DNS-PUBLISH**: publish a serve instance's tailcat address to a Cloudflare TXT record. New `dns_publish` flag + `dns_name` field on serve instances (overview.js + services.js); `general.cf_api_token`/`cf_zone_id` credentials on the Overview page. `tailcat-dns-publish.sh` writes `tailcat=<addr>` (bare value, no quotes) at the FQDN; init.d publishes after the address file populates and unpublishes on stop/reload.
